...
export default function SubjectNotifyPage() {
  ...
  const slug = (params?.subject as string)?.toLowerCase() ?? ''
  const label = SUBJECT_LABELS[slug]

  const [email, setEmail] = useState('')
  const [agreed, setAgreed] = useState(false)
  const [status, setStatus] = useState<'idle' | 'loading' | 'success' | 'exists' | 'error'>('idle')

  // Unknown subject — shouldn't normally happen
  if (!label) {
    return (
      <div className="min-h-screen bg-background">
        <Navbar />
        <main className="mx-auto max-w-lg px-4 pt-24 pb-16 text-center">
          <p className="text-sm text-muted-foreground">Subject not found.</p>
          <a href="/notify" className="mt-4 inline-flex text-sm underline">Back</a>
        </main>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-lg px-4 pt-24 pb-16">
        <div className="space-y-6">
          <header>
            <a href="/notify" className="mb-4 inline-flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground">
              <svg className="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
              </svg>
              All subjects
            </a>
            <p className="mb-2 text-xs font-medium uppercase tracking-wider text-primary">Notifications · {label}</p>
            <h1 className="text-2xl font-semibold text-foreground leading-tight">
              Stay in the loop
            </h1>
            <p className="mt-2 text-sm text-muted-foreground">{SUBJECT_DESC[slug]}</p>
          </header>
          ...
          <div className="rounded-xl bg-white p-5 shadow-sm ring-1 ring-border/50">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                <h2 className="text-sm font-semibold text-foreground">Join the {label} WhatsApp group</h2>
                <p className="mt-0.5 text-xs text-muted-foreground">
                  Get reminders and updates directly in WhatsApp. Tap once to join.
                </p>
                <a
                  href={WHATSAPP_LINKS[slug]}
                  target="_blank"
          ...
          <div className="rounded-xl bg-white p-5 shadow-sm ring-1 ring-border/50">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="mt-0.5 text-xs text-muted-foreground">
                  One email before each {label} session. No spam.
                </p>
                ...
                {status === 'success' || status === 'exists' ? (
                  <div className="mt-3 flex items-center gap-2">
                    ...
                    <p className="text-sm font-medium text-foreground">
                      {status === 'exists' ? `Already subscribed to ${label}` : `You're on the ${label} list`}
                    </p>
                  </div>
                ) : (
                  <form onSubmit={handleSubmit} className="mt-3 space-y-3">
                    ...
                    <label className="flex items-start gap-2.5 cursor-pointer">
                      ...
                      <span className="text-xs text-muted-foreground leading-relaxed">
                        I agree to receive {label} session reminders. I can{' '}
